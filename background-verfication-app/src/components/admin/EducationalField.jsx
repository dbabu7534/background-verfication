...
const EducationalField = () => {
 const [fieldBox, setFieldBox] = useState([]);

  useEffect( () => {
          const fetchData = async () => {
              try {
                  const response = await ApiService.getEducationalField();
                  console.log(response.data);
  
                  if (response.status === 200 && response.data.educationalField) {
                      setFieldBox(response.data.educationalField);
                  }
              } catch (error) {
      ...
      const handleSubmit = async () => {
          try {
              ...
              const response = await ApiService.putEducationalField(fieldBox);
